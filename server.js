...
app.post('/api/onboarding/structure', requireAuth, async (req, res) => {
  try {
    ...
    // Asset Class is a link field, so we need record ids. Fetch the table once.
    const acRes = await fetch(`https://api.airtable.com/v0/${baseId}/Asset%20Classes`, { headers });
    const acJson = await acRes.json();
    const classId = {};
    (acJson.records || []).forEach(r => {
      const n = (r.fields['Asset Class Name'] || '').toUpperCase().trim();
      if (n) classId[n] = r.id;
    });
    ...
    if (nComp > 0) {
      const label = compressorType === 'recip'
                  ? 'RECIPROCATING COMPRESSOR'
                  : 'ROTARY SCREW COMPRESSOR';
      // Compressor Type options in Airtable: SCREW / RECIP STANDARD / RECIP XL
      const ctype = compressorType === 'recip' ? 'RECIP STANDARD' : 'SCREW';
      plan.push({
        system: 'Compressors',
        crit: 'MAJOR',
    ...
    sorted.forEach(st => {
      ...
      if (liquidFeed === 'pumped') {
        assets.push({ name: `${st.label} Recirc Pump 1`, type: 'PUMP', cls: 'REFRIGERANT PUMP' });
        assets.push({ name: `${st.label} Recirc Pump 2`, type: 'PUMP', cls: 'REFRIGERANT PUMP' });
      }
      if (liquidFeed === 'pumper') {
        assets.push({ name: `${st.label} Pumper Drum`, type: 'VESSEL', cls: 'PRESSURE VESSEL' });
      }
      const nEvap = Math.max(0, parseInt(st.evap, 10) || 0);
      for (let i = 0; i < nEvap; i++) {
        assets.push({ name: `${st.label} Evap ${i + 1}`, type: 'EVAPORATORS', cls: 'AIR COOLED EVAPORATOR' });
      }
    ...
    if (plantType === 'chiller') {
      plan.push({
        ...
        assets: [
          ...
          { name: 'Glycol Pump 1', type: 'PUMP', cls: 'REFRIGERANT PUMP' },
          { name: 'Glycol Pump 2', type: 'PUMP', cls: 'REFRIGERANT PUMP' },
          { name: 'Glycol Pump 3', type: 'PUMP', cls: 'REFRIGERANT PUMP' }
        ]
      });
    }
    ...
    for (const block of plan) {
      ...
      for (const a of block.assets) {
        ...
        const acId = classId[(a.cls || '').toUpperCase().trim()];
        if (acId) assetFields['Asset Class'] = [acId];
        else console.warn('[ONBOARD] no Asset Class match for', a.cls);

        if (a.ctype) assetFields['Compressor Type'] = a.ctype;

        const assetRec = await created('Assets', assetFields);
...
app.post('/api/onboarding/asset', requireAuth, async (req, res) => {
  try {
    ...
    if (!r.ok) {
      const detail = (j && j.error && (j.error.message || j.error.type)) || JSON.stringify(j);
      console.error('[ONBOARD] asset patch failed:', detail, 'fields sent:', JSON.stringify(fields));
...
app.get('/api/rubric', requireAuth, async (req, res) => {
  try {
    ...
    function buildAnswers(f, qType) {
      if (qType === 'CL') {
        const d = k => (f['Deduct ' + k] != null ? f['Deduct ' + k] : null);
        const out = [
          { label:'Excellent',    desc: f['Answer Excellent']    || '', score: 100, deduct: d('Excellent') },
          { label:'Good',         desc: f['Answer Good']         || '', score: 75,  deduct: d('Good') },
          { label:'Fair',         desc: f['Answer Fair']         || '', score: 50,  deduct: d('Fair') },
          { label:'Degraded',     desc: f['Answer Degraded']     || '', score: 25,  deduct: d('Degraded') },
          { label:'Needs Repair', desc: f['Answer Needs Repair'] || '', score: 0,   deduct: d('Needs Repair') }
        ];
        // Onboarding/history questions can carry an explicit Unknown answer.
        if (f['Answer Unknown'] || f['Deduct Unknown'] != null) {
          out.push({
            label: 'Unknown',
            desc:  f['Answer Unknown'] || 'Unknown — no records available',
            score: null,
            deduct: f['Deduct Unknown'] != null ? f['Deduct Unknown'] : null,
            unknown: true
          });
        }
        return out;
